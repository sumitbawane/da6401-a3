...
from model_vanilla import evaluate_model, create_model_from_preprocessor, create_dataloaders
...
def train_with_wandb(config=None):
    ...
    with wandb.init(config=config):
        ...
        # Create model with our hyperparameters
        ...
        for epoch in range(config.epochs):
            ...
            for batch_idx, batch in train_bar:
                ...
                # Forward pass
                output = model(src, trg, teacher_forcing_ratio=config.teacher_forcing_ratio,current_epoch=epoch,total_epochs=config.epochs)
            ...
            # Validation phase
            model.eval()
            #train_char_acc, train_word_acc = evaluate_model(model, train_loader, preprocesor,beam_size=config.beam_size)
            char_acc, word_acc = evaluate_model(model, val_loader, preprocesor,beam_size=config.beam_size)
            ...
            if word_acc > best_val_accuracy:
              ...
              # log as a WandB artifact (this copies, no symlinks)
               artifact = wandb.Artifact(name="best-model",type="model",metadata={'val_accuracy': word_acc})
               artifact.add_file(model_path)
               wandb.log_artifact(artifact)
            

def main():
    parser = argparse.ArgumentParser(description='Transliteration Model Training with wandb')
